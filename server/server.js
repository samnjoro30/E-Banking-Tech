...
const sessionMiddleware = require('./utils/session.util');
const { redis, connectAllRedis } = require('./utils/redis/redis.util');
const { checkRedis } = require('./utils/redis/redis.health');
...
async function startServer() {
  ...
  app.get('/health', async (req, res) => {
    ...
    const redisHealth = await Promise.all([
      checkRedis(redis.session, 'session'),
      checkRedis(redis.rateLimit, 'rateLimit'),
      checkRedis(redis.queue, 'queue'),
    ]);
    const response = {
...
startServer().catch(err => {
  console.error('Starting failed', err);
  process.exit(1);
});
